...
(function(){
  ...
  const scenes = [
    [0,     5000, '夜晚，一轮明月挂在天上。<br>小明走在回家的路上……', 'night'],
    [5000,  10000,'路面有的地方亮闪闪，有的地方黑乎乎。<br><span class="reveal">迎着月光走——亮的是水，暗的是路。</span>', 'toward'],
    [10000, 16000,'可是调个头，背着月光走呢？<br><span class="reveal">这时候，暗的地方反而是水！</span>', 'away'],
    [16000, 21000,'同样是月光照在水面和路面上，<br>为什么换个方向走，亮的暗的就对调了？', 'question'],
    [21000, 26000,'秘密就在——<span class="reveal">光的反射</span>。<br>跟着下面的实验一起找答案吧！', 'reveal'],
  ];
  ...
  function person(ctx,x,y,s,dir,phase){
    ...
    ctx.strokeStyle='#f4f7ff'; ctx.lineWidth=3.5; ctx.lineCap='round';
    const sw=Math.sin(phase)*0.5;
    ctx.beginPath(); ctx.arc(0,-34,7,0,7); ctx.stroke();
    ctx.beginPath(); ctx.moveTo(0,-27); ctx.lineTo(0,-8); ctx.stroke();
    ctx.beginPath(); ctx.moveTo(0,-8); ctx.lineTo(8*sw+3,8); ctx.stroke();
    ctx.beginPath(); ctx.moveTo(0,-8); ctx.lineTo(-8*sw-3,8); ctx.stroke();
    ctx.beginPath(); ctx.moveTo(0,-24); ctx.lineTo(7*sw,-14); ctx.stroke();
    ctx.beginPath(); ctx.moveTo(0,-24); ctx.lineTo(-7*sw,-14); ctx.stroke();
    ctx.restore();
  }
  ...
  function start(){ if(raf) cancelAnimationFrame(raf); t0=null; raf=requestAnimationFrame(drawFrame); }
